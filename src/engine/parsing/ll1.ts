import { Grammar, Production, ParsingTable } from "../types";
    ...
    for (const a of firstAlpha) {
      if (a !== "ε") {
        if (table[A][a]) {
          ...
          conflicts.push(
            `Conflict at M[${A}, ${a}]: Existing entry ${formatProd(table[A][a][0])}, trying to add ${formatProd(prod)}`,
          );
          table[A][a].push(prod);
        } else {
          table[A][a] = [prod];
          ...
          if (table[A][b]) {
            ...
            if (
              !existing.some(
                (p) =>
                  p.lhs === prod.lhs &&
                  JSON.stringify(p.rhs) === JSON.stringify(prod.rhs),
              )
            ) {
              conflicts.push(
                `Conflict at M[${A}, ${b}]: Existing entry ${formatProd(existing[0])}, trying to add ${formatProd(prod)}`,
              );
              existing.push(prod);
            }
          } else {
